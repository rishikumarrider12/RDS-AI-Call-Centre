...
import type { Call, CallTranscriptLine, CallEvent } from '@rds/types'
import { recordAudit } from '../lib/audit'

export class CallService {
  private repository = new CallRepository()

  private toCall(dbCall: any): Call {
    return {
      id: dbCall.id,
  ...
  async list(
    ...
    return {
      data: result.calls.map((c) => this.toCall(c)),
      total: result.total,
      page: result.page,
  ...
  async getById(organizationId: string, id: string): Promise<Call> {
    const dbCall = await this.repository.findById(organizationId, id)
    if (!dbCall) throw new Error('Call not found')

    const call = this.toCall(dbCall)

    if (dbCall.contact_id) {
      const contact = await this.repository.getContact(dbCall.contact_id)
      if (contact) {
        call.contactName = [contact.first_name, contact.last_name].filter(Boolean).join(' ') || null
        call.contactPhone = contact.phone
      }
    }
    if (dbCall.campaign_id) {
      const campaign = await this.repository.getCampaign(dbCall.campaign_id)
      call.campaignName = campaign?.name ?? null
    }
    if (dbCall.agent_id) {
      const agent = await this.repository.getAgent(dbCall.agent_id)
      call.agentName = agent?.name ?? null
    }

    const transcripts = await this.repository.getTranscripts(id)
    call.transcriptLines = transcripts.map(
      (t): CallTranscriptLine => ({
        sequence: t.sequence,
        channel: t.channel,
        text: t.text,
  ...
  async getActive(organizationId: string): Promise<Call[]> {
    const rows = await this.repository.active(organizationId)
    return rows.map((c) => this.toCall(c))
  }

  async startCall(organizationId: string, actorId: string, input: Record<string, unknown>): Promise<Call> {
  ...
  async endCall(organizationId: string, actorId: string, id: string): Promise<Call> {
    const dbCall = await this.repository.findById(organizationId, id)
    if (!dbCall) throw new Error('Call not found')
    ...
    const call = this.toCall(updated)
    await recordAudit({
      organizationId,
      action: 'call.end',
      actorId,
  ...
  async pauseCall(organizationId: string, actorId: string, id: string): Promise<Call> {
    const dbCall = await this.repository.findById(organizationId, id)
    if (!dbCall) throw new Error('Call not found')
  ...
  async resumeCall(organizationId: string, actorId: string, id: string): Promise<Call> {
    const dbCall = await this.repository.findById(organizationId, id)
    if (!dbCall) throw new Error('Call not found')
  ...
  async transferCall(organizationId: string, actorId: string, id: string, toAgentId: string): Promise<Call> {
    const dbCall = await this.repository.findById(organizationId, id)
    if (!dbCall) throw new Error('Call not found')
  ...
  async getTranscript(organizationId: string, id: string): Promise<CallTranscriptLine[]> {
    const dbCall = await this.repository.findById(organizationId, id)
    if (!dbCall) throw new Error('Call not found')
    const transcripts = await this.repository.getTranscripts(id)
  ...
  async getEvents(organizationId: string, id: string): Promise<CallEvent[]> {
    const dbCall = await this.repository.findById(organizationId, id)
    if (!dbCall) throw new Error('Call not found')
